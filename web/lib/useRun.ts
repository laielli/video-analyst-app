 ...
 * permalink replay (a stored run addressed by `runId`). `useRun.run` accepts any; a bare `run()`
 * with no arg replays the hook's default canned query (the auto-play URL — D-DR7). A permalink
 * replay is just another SSE stream (the accumulate logic is identical).
 */
...
/**
 * Build the shareable permalink for a stored run: `${origin}/?run=<id>`. Pure (no clipboard, no
 * window) so it is unit-testable; the page passes `window.location.origin`.
 */
export function runLink(origin: string, runId: string): string {
