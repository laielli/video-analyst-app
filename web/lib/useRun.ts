...
/** True iff this run-doc could not ground an answer (Q1 -> A). */
export function isUngrounded(findings: Findings | null): boolean {
  return !!findings && findings.grounded === false;
}
 ...
 *
 * `run()` with no args replays the hook's default canned `queryId` (D-DR7 auto-play);
 * `run({ kind: "free", text, clip })` compiles + replays a typed question instead.
 */
export function useRun(queryId: string | null, paceMs = 1200) {
  ...
  const run = useCallback((req?: RunRequest) => {
    // Default to the canned auto-play query when called with no explicit request (D-DR7).
