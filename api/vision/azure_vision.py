...
`detect` (OBJECTS), `read_text` (READ / OCR), and `describe_scene` (CAPTION). Stateless: one
image in, structured results out. Boxes are returned in NORMALIZED 0-1 coordinates (relative to
the analyzed frame), which is exactly the overlay contract the UI consumes
(overlay-normalized-stills decision) — so the same payload feeds the interpreter, the run-doc
cache, and the canvas. CAPTION returns one whole-image caption (no region), so we store a
full-frame box for overlay parity.
...
@dataclass
class Caption:
    """One whole-image scene caption. CAPTION returns no region, so `box` is the full frame
    (Box(0,0,1,1)) for overlay parity with detect/read_text."""
    text: str
    confidence: float | None
    box: Box
...
class AzureVision:
    ...
    def analyze(self, image_bytes: bytes, detect: bool = True, read: bool = True,
        ...
        if cap is not None and getattr(cap, "text", None):
            conf = getattr(cap, "confidence", None)
            captions.append(
                Caption(
                    text=cap.text,
                    confidence=round(conf, 4) if conf is not None else None,
                    box=Box(0.0, 0.0, 1.0, 1.0),  # CAPTION has no region -> full-frame box
                )
            )
