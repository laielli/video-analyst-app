...
class FakeAzureVision:
    ...
    def __init__(self, objects=None, caption_text="A football match scene"):
    ...
    def analyze(self, image_bytes, detect=True, read=False, caption=False):
        ...
        return AnalysisResult(
            ...
            lines=[],
            # describe_scene precompute: one whole-image caption + full-frame box when requested.
            captions=[Caption(text=self.caption_text, confidence=0.8, box=Box(0.0, 0.0, 1.0, 1.0))]
            if caption else [],
        )
