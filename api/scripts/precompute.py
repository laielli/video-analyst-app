...
from limits import MAX_CAPTION_LEN  # noqa: E402 — caption free-text cap
...
# --------------------------------------------------------------------------------------
# describe_scene (Layer 3 CAPTION) — extract frame -> AzureVision.analyze(caption=True)
# --------------------------------------------------------------------------------------

def run_describe_scene(manifest: dict, timestamps: list[int], *, frame_provider, vision) -> dict:
    """Returns a captions map keyed by sampled ts ms (mirrors run_detect's detect map). Gated by
    the manifest opt-in sampling.caption_frames (default false), so a default manifest emits no
    captions. vision is an AzureVision-like with .analyze(bytes, detect=False, read=False,
    caption=True). Each caption's free text is truncated to MAX_CAPTION_LEN at this boundary."""
    ...
    for ts in timestamps:
        ...
        entries = []
        for c in getattr(result, "captions", []) or []:
            b = c.box.rounded() if hasattr(c.box, "rounded") else c.box
            box = {"x": b.x, "y": b.y, "w": b.w, "h": b.h} if hasattr(b, "x") else dict(b)
            text = (c.text or "")[:MAX_CAPTION_LEN]
            if text:
                entries.append({"text": text, "confidence": c.confidence, "box": _round_box(box)})
        if entries:
...
def canonical_cache(clip: dict, detect: dict, read_text: dict, events: list[dict],
    ...
    `captions` slice (keyed by sampled ts ms, mirroring `detect`) is keyword-defaulted to None so
    existing callers stay valid; it is emitted ONLY when present (describe_scene opt-in)."""
    sorted_detect = {}
...
def validate_cache_shape(cache: dict) -> list[str]:
    ...
    # (length <= MAX_CAPTION_LEN), a confidence, and a `box` in [0,1].
...
def run_precompute(
    ...
    *,
    ...
    # --- describe_scene captions (manifest opt-in sampling.caption_frames, default off) ---
