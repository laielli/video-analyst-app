...
# a frozenset for "any of this explicit allow-list", or "any" for any synthesizable kind —
# used by `answer`, which op_answer now generalizes over collections + `number` + `ordered`
# (Phase 0). Without "any", a `count -> answer` program would not validate because `number` is
# not a collection. temporal_order takes a frozenset allow-list (NOT None): op_temporal_order
# keys on detection `det_id` (primitives.py), so it is meaningless over `captions`/`number` —
# the allow-list closes that off after `captions` joined COLLECTION_KINDS.)
EXPECTS = {
...
# Iterable collection kinds (filter/count accept any of these; answer accepts these + number +
# ordered). `captions` is a member so count(describe_scene(...))/filter compose, but it is
# DELIBERATELY excluded from temporal_order's allow-list (above) — that op is det_id-keyed.
...
def _arg_domain_errors(i: int, sid: str, op: str, args: dict) -> list[str]:
    ...
    if op == "detect":
        ...
        # Optional max_captions arg: when present, bound it to 1..MAX_SCENE_CAPTIONS so a program
        # cannot request thousands of per-frame captions (mirrors detect.classes count cap).
        ...
        if mc is not None:
            ...
            elif not (1 <= mc <= MAX_SCENE_CAPTIONS):
                out.append(
    ...
    for i, step in enumerate(program):
        ...
        for field, want in EXPECTS.get(op, {}).items():
            ...
            if want == "any":
                ...
                # An explicit kind allow-list (e.g. temporal_order accepts only det_id-bearing
                # collections, never captions/number). Reject anything outside it.
                if got not in want:
                    errors.append(
